...
export var payments = crossfilter<Payment>([
...
var addFn = (p: Group, v: Payment) => {
    p.total += v.total;
    p.count += 1;
    return p;
};
var remFn = (p: Group, v: Payment) => {
    p.total -= v.total;
    p.count -= 1;
    return p;
};
...
export var paymentsByTotal = payments.dimension(d => d.total);
export var paymentVolumeByTotal = paymentsByTotal.group<number,  Group>()
    .reduce(addFn, remFn, initFn)
    .order(value => value.total);

export var paymentsByType = payments.dimension(d => d.type);
export var paymentVolumeByType = paymentsByType.group<number,  Group>()
    .reduce(addFn, remFn, initFn)
    .order(value => value.total);
